...
void drawLissajous(int steps = 1000, int repeats = 1) {
  Serial.println("Drawing Lissajous Curve");
  const int A_base = 2047;
  const int B_base = 2047;
  const int a = 3;
  const int b = 2;
  const float delta_base = PI / 2;

  const float dt = TWO_PI / steps;

  for (int r = 0; r < repeats; r++) {
    for (int l = 0; l < speed; l++) {
      float delta = delta_base + (PI / 4) * sin(TWO_PI * l / speed); // Modulate phase smoothly
      float hue = float(l) / speed; // Change hue based on l
      laser.setColor(laser.hsvToRgb(hue, 1.0, 1.0));

      for (float t = 0; t < TWO_PI; t += dt) {
        int x = A_base * sin(a * t + delta) + 2047;
        int y = B_base * sin(b * t) + 2047;
        laser.SendToRaw(x, y);
      }
    }
  }
